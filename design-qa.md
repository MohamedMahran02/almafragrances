...
| Link integrity | Customer-facing actions must not use empty, placeholder, unresolved resource or missing-anchor destinations | Homepage links resolve to explicit routes or existing anchors; the unavailable gift-card product route is replaced by the permanent Gifting by ALMA section anchor across header, service and footer entry points | Pass |
...
- Gifting-route checks: header Gifting, Personal gifting → Explore gifting and footer Gifting all resolve to `/#shopify-section-gifting_story`; the generated homepage contains that exact section anchor, and no rendered navigation or service link retains the unavailable `/products/alma-fragrances-gift-card` destination.
