...
- `sections/alma-service-band.liquid`: restored descriptive service band for three factual ALMA destinations: gift cards, the three-step ritual and the complete catalog. It uses burgundy star markers, headings, descriptions and separate underlined links in three divided desktop columns and stacked mobile cards. It does not invent shipping, samples, returns or client-service promises.
...
- `sections/footer.liquid` and `assets/alma-footer.js`: retain the compact reference-inspired composition while applying ALMA's warm neutral and burgundy palette. The exact logo/UAE context, original Shop, Explore ALMA, Help and Legal data, native customer newsletter form, and burgundy copyright/legal row are present. The four groups remain expanded on desktop and become native `details` disclosures below 750 px. Contact links normalize Shopify's internal page URI to `/pages/contact`; localization, payment and configured-social hooks remain available without inventing empty icons or claims.
