...
- The theme now contains a complete Arabic storefront locale alongside English, Arabic versions of the current custom homepage and policy copy, RTL document direction and a globe language control in the header. English product and collection description filtering is retained; Arabic removes standalone English paragraphs from bilingual descriptions when Arabic paragraphs exist. The Shopify Admin locale has **not yet been enabled or published**, and product, collection, option, menu, metaobject, checkout and system translations have **not yet been saved** in Shopify. The storefront is therefore not verified as fully Arabic, and the header control appears only after Shopify makes a second language available. The Shopify store currency is United Arab Emirates Dirham (AED), and theme currency-code display is enabled. Storefront prices omit redundant leading `Dhs.`; Arabic theme output replaces the visible `AED` code with `د.إ`.
...
The theme now includes a complete Arabic storefront locale, current homepage and custom policy translations, a header globe switch and RTL styling. Shopify Admin Arabic publication and live catalog/checkout/system translations remain outstanding because Admin login was unavailable. Do not claim that Arabic is selectable or free of English on the live storefront until the Admin locale and resource translations are saved and verified. Currency-code display is enabled and the authenticated Shopify store currency was changed from EGP to AED on 2026-09-13. Numeric product prices and shipping rates were not converted automatically and must be reviewed before production.
...
Latest change: the theme-side English/Arabic switch, Arabic locale and custom content translation are implemented, but the store still needs Arabic publication and translated Shopify resources. Obtain authenticated Admin access, complete translations of products, collections, options, menus, metaobjects, pages, checkout and system text, then inspect both languages on the connected theme. Preserve the tightened spacing and wide Dukhoon composite from the previous work. Typography option 2 (Cormorant Garamond + Jost) remains the active temporary English comparison candidate; Arabic uses system Arabic-capable fonts until an approved Arabic font is selected.
```
