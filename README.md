...
- Website identity: the homepage opening header uses the supplied transparent wordmark, centered above desktop navigation and centered between the mobile controls. Over the first 132 px of desktop scroll, the exact supplied `assets/alma-logo-morph.webm` is scrubbed from the wordmark to the icon while the mark moves left and scales down. Mobile/tablet scrub the same morph over 96 px but keep the mark centered throughout; spatial movement is reserved for desktop and larger screens. Stopping scroll freezes the current frame. The video is never autoplayed, and the supplied static wordmark/icon remain the fallback when usable WebM data is unavailable. All source artwork remains exact and product packaging is unchanged.
...
- `snippets/alma-logo.liquid`, `assets/alma-logo-icon.png`, `assets/alma-logo-wordmark.png` and `assets/alma-logo-morph.webm`: exact supplied transparent artworks. The 1080×1080, 2.2-second WebM performs the wordmark-to-icon shape morph; JavaScript keeps it paused and maps scroll progress directly to `currentTime`. On the desktop homepage, the 132.5 px opening header centers the wordmark above navigation; scroll 0–132 px moves/scales the morph stage left, lifts the navigation and contracts the visible header to 72 px. Mobile/tablet use the same video timeline over 0–96 px but keep the complete morph centered in their fixed-height control row. Static assets provide a graceful fallback until video data loads or on unsupported browsers. A fixed-size transparent sticky wrapper prevents header contraction from changing document scroll position, so stopping scroll also stops the morph. While the mobile menu is open below 990 px, the header is fixed to the current viewport so Dawn's drawer cannot fall back to its original document position. `header.liquid` retains Dawn's utility controls, menu drawer, app blocks, account and cart.
