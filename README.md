...
- English is the only customer-facing language in the current theme. Arabic source copy remains in the existing Shopify catalog for a later dedicated Arabic locale, but the English storefront filters it from product and collection descriptions. The store currency still requires an authenticated Shopify Admin change from EGP to AED; theme currency-code display is already enabled.
...
Homepage: single-row utility header with a larger left-aligned transparent ALMA icon, centered category navigation and right-side search/account/cart utilities → viewport-filling campaign hero with an overlaid CTA and a short pinned opening-scroll stage → ritual introduction → tabbed fragrance carousel with four products visible and 15 fragrance products in its combined tab → 50/50 layering editorial feature with three product steps → full-width Dukhoon image and centered story → four-product gifting carousel → horizontally scrollable six-category ALMA wardrobe → compact three-part ALMA experience band → split newsletter/help footer. The header remains sticky and contracts from its full logo state to a compact bar while the hero is pinned, then remains available as later sections enter. Product and wardrobe rails include inset transparent previous/next controls and progress indicators. The announcement strip is intentionally removed. Hero and ritual remain inside the same `image_banner` section ID.
...
- `sections/alma-campaign.liquid`: replaces the section type at `index.sections.image_banner`. Campaign image and CTA appear inside a sticky hero frame before the ritual heading, body and link. The initial frame exactly fills the viewport below the full header; desktop retains it for 120 px of scroll while the header contracts, keeping the CTA at the same bottom-centred position, then releases into the ritual and remaining sections. Mobile uses the same complete first-screen composition without the extra pinned scroll stage. Existing block IDs/order `[heading, button]` remain. The hero uses an editable image picker with `assets/alma-campaign-hero.png` as fallback.
...
- `assets/alma-theme.css`: responsive campaign/grid, regular Georgia serif headings, native body font, white/soft-gray surfaces, rectangular CTA and a high-resolution transparent logo window with breathing room around every visible stroke. The landing hero uses `vh`/`svh` sizing against Dawn's measured header height, separate desktop/mobile focal crops and an explicit sticky-header layer. Horizontal page overflow is suppressed only on the root `html` element so the document retains one main vertical scrollbar. Mobile uses a central hero crop and one product column.
...
The current storefront locale is English-only; preserve Arabic source copy for a future separate Arabic locale. Currency-code display is enabled, but the store currency is still EGP and must be changed to AED by an authenticated store owner in Shopify Admin before production.
The opening campaign uses a viewport-filling sticky frame: the CTA is visible immediately, the desktop header contracts while the hero holds for 120 px of scroll, and the existing later sections then continue normally. Preserve this interaction unless the user requests another landing behavior.
