...
### 2026-09-24 - Center selected mobile Find Your Aura results

- Request: prevent the mobile popup from attaching to the top after selecting a scent.
- Changed: added a selected-result-only center position using CSS `translate`, so the content-sized dialog remains centered and grows above and below its midpoint without modifying the close handler.
- Files: `assets/alma-theme.css`, `README.md`.
- Checks: selected-result positioning and close-flow isolation review, plus `git diff --check` pass.
