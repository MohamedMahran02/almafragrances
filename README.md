...
Latest correction: homepage vertical spacing remains tighter, while Dukhoon again uses its original full-width desktop photo panel followed by centered story. The portrait Shopify photo is fitted inside the panel to remove the excessive zoom; mobile retains the existing cover crop and title overlay. The connected Shopify theme CSS was pulled back and matches the repository after line-ending normalization; visually inspect the rendered theme at desktop and mobile widths. Next requested work: perform final owner review of AED price presentation, the simplified header and repaired storefront destinations on the connected Shopify theme. Typography option 2 (Cormorant Garamond + Jost) remains the active temporary comparison candidate until the user selects it or asks for option 3 (Instrument Serif + Inter).
```
