...
- Website identity: the homepage opening header uses the supplied transparent wordmark, centered above desktop navigation and centered between the mobile controls. Over the first 132 px of desktop scroll or 96 px of mobile/tablet scroll, the exact supplied `assets/alma-logo-morph.webm` is scrubbed from the wordmark to the icon while the mark moves left and scales down; stopping scroll freezes its current frame. The video is never autoplayed, and the supplied static wordmark/icon remain the fallback when usable WebM data is unavailable. All source artwork remains exact and product packaging is unchanged.
