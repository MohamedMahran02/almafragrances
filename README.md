...
- Desktop 1365×617, compact mobile 316×599 and standard mobile 390×844 checked for the latest landing behavior. No horizontal overflow or broken visible images. The CTA is visible immediately, remains fixed through the hero scroll where the image exceeds the available viewport, and releases with its responsive media inset. The mobile menu was also verified after scrolling: it opens at the current viewport, fills the space below the fixed header and closes normally. Earlier tab selection by click/keyboard checks remain documented. The latest local browser console check has no errors or warnings.
