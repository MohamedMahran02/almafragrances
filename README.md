...
Homepage: single-row utility header with a larger left-aligned transparent ALMA icon, centered category navigation and right-side search/account/cart utilities → original wide campaign artwork with a viewport-tracking CTA → compact centered ritual introduction → tabbed fragrance carousel with four products visible and 15 fragrance products in its combined tab → 50/50 layering editorial feature with three product steps → full-width Dukhoon image and centered story → four-product gifting carousel → six-category ALMA wardrobe (four-card carousel on desktop, two-column grid on mobile) → compact one-row ALMA experience image links → compact all-black footer with ALMA branding, Shop and Customer care navigation, newsletter and a slim legal row. The campaign is restored to its original 56.25vw presentation instead of being cropped into a viewport-height frame. Its CTA is visible on the first screen and remains bottom-centred while the artwork scrolls on both desktop and compact mobile viewports, then releases with a 50 px desktop or 30 px mobile inset before the image boundary. The header remains sticky and contracts from its full logo state to a compact bar. Product rails and the desktop wardrobe rail include inset transparent previous/next controls and progress indicators; the mobile wardrobe is a static grid without carousel controls. The three ALMA experience destinations stay in one compact row at every breakpoint. Footer navigation changes from two visible desktop columns to two closed native disclosures on mobile. The announcement strip is intentionally removed. Hero and ritual remain inside the same `image_banner` section ID.
...
- `sections/alma-service-band.liquid`: compact Diptyque-service-inspired row for three factual ALMA destinations: gift cards, the three-step ritual and the complete catalog. Each whole card is a link with a curated 1000 px product image, concise title and optional Theme Editor image override. Descriptions and separate CTA lines remain saved in the section blocks but are intentionally omitted from the rendered compact band. It does not invent shipping, samples, returns or client-service promises.
...
- `sections/footer.liquid` and `assets/alma-footer.js`: render the complete footer as a compact near-black reference-inspired composition: exact ALMA logo/UAE context, Shop and Customer care navigation, native customer newsletter form, and a divided copyright/legal row. The two navigation groups remain expanded on desktop and become native `details` disclosures below 750 px. Contact links normalize Shopify's internal page URI to `/pages/contact`; localization, payment and configured-social hooks remain available without inventing empty icons or claims.
