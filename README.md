...
- `sections/alma-editorial-feature.liquid`: reusable Diptyque-referenced image-and-copy feature with split or image-led layouts, left/right media, optional live featured product/price and up to three linked product steps. At desktop widths, the editable eyebrow and heading use the original clean content-panel placement: beside the split layering photograph and beside the fully visible Dukhoon photo. Below 750 px, those same values become code-native photo overlays: bottom-left for layering and 80 px from the top for Dukhoon, each with a localized contrast gradient that does not obscure the product. Dukhoon uses its Shopify featured product image by default, with a Theme Editor image override taking precedence. The portrait image fits without cropping in a dark desktop media panel; mobile retains its 500 px image and title overlay. Supporting copy, live product/price, actions and ritual steps remain in the content panel; all text, products, images, links and treatments remain editable.
...
- Files: assets/alma-theme.css and this README.
- Checks: the catalog snapshot Dukhoon source image measures 870 × 1080; static CSS inspection confirms desktop contain and mobile cover rules. CSS braces are balanced, Git diff whitespace validation passes, and Theme Check passes with 0 errors and 9 existing warnings. Rendered browser and connected Shopify theme verification remain outstanding because no browser surface is available in this session. Publication status is unverified.
...
Latest change: homepage vertical spacing is tighter, and the Dukhoon Shopify portrait photo is fully contained beside its story on desktop; mobile retains the existing cover crop and title overlay. Verify this on the connected Shopify theme at desktop and mobile widths. Next requested work: perform final owner review of AED price presentation, the simplified header and repaired storefront destinations on the connected Shopify theme. Typography option 2 (Cormorant Garamond + Jost) remains the active temporary comparison candidate until the user selects it or asks for option 3 (Instrument Serif + Inter).
```
