...
- The theme now contains a complete Arabic storefront locale alongside English, Arabic versions of the current custom homepage and policy copy, RTL document direction and a globe language control in the header. English product and collection description filtering is retained; Arabic removes standalone English paragraphs from bilingual descriptions when Arabic paragraphs exist. Arabic is now added to the Shopify Admin store domain but remains **unpublished**. Shopify Translate & Adapt is installed and its store-wide Arabic auto-translation job is running; live product, collection, option, menu, metaobject, checkout and system translations have not yet been verified. The storefront is therefore not verified as fully Arabic, and the header control appears only after Arabic is published. The Shopify store currency is United Arab Emirates Dirham (AED), and theme currency-code display is enabled. Storefront prices omit redundant leading `Dhs.`; Arabic theme output replaces the visible `AED` code with `د.إ`.
...
The theme now includes a complete Arabic storefront locale, current homepage and custom policy translations, a header globe switch and RTL styling. Shopify Admin Arabic was added to the store domain on 2026-09-20 and remains unpublished. Shopify Translate & Adapt is installed and its store-wide auto-translation job is running; it may take several hours. Do not claim that Arabic is selectable or free of English on the live storefront until the job completes, unsupported fields are translated, Arabic is published and the result is verified. The generated Shopify Privacy Policy requires manual translation; legal copy requires owner review. Currency-code display is enabled and the authenticated Shopify store currency was changed from EGP to AED on 2026-09-13. Numeric product prices and shipping rates were not converted automatically and must be reviewed before production.
...
Latest change: authenticated Admin access is available, Arabic has been added to the domain, and Shopify's store-wide Arabic auto-translation is processing. Wait for completion, review and fill products, collections, options, menus, metaobjects, pages, checkout and system text, manually translate unsupported policy content, then publish and inspect both languages on the connected theme. Preserve the tightened spacing and wide Dukhoon composite from the previous work. Typography option 2 (Cormorant Garamond + Jost) remains the active temporary English comparison candidate; Arabic uses system Arabic-capable fonts until an approved Arabic font is selected.
```
