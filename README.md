...
- Public visual review URL: `https://alma-fragrances-preview.vercel.app`. Vercel Authentication and password protection are disabled for this project, so the site is accessible without a Vercel account. The URL is a visual homepage preview, not a Shopify commerce deployment. Its static build intentionally receives an empty catalog and never publishes saved product names, images or prices; live products remain Shopify-only.
...
- Checks: static build, catalog-leak assertions, JavaScript syntax, Theme Check, desktop/mobile visual smoke tests and Git whitespace are run before handoff. The Vercel surface remains a non-commerce design preview; real catalog data and commerce behavior must be reviewed through Shopify when a store is available.
