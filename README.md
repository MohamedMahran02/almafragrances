...
- Verified test store: `https://dk5qhx-ax.myshopify.com/`. The user confirms `main` was connected to its Shopify theme. The Shopify trial ended by 2026-09-16, so this store is no longer the active review surface. The repository now also builds a non-commerce Vercel visual preview for shareable design review; Shopify remains authoritative for eventual production commerce and Theme Editor behavior.
...
- `sections/footer.liquid`: retains the compact reference-inspired black-and-ivory composition. The exact logo artwork, UAE/English context, copyright, original Shop, Explore ALMA, Help and Legal data and native customer newsletter form are present. Payment badges, including PayPal, are intentionally not rendered. All four navigation groups stay visibly open in a four-column desktop layout and a two-column mobile grid; the obsolete disclosure script was removed. Contact links normalize Shopify's internal page URI to `/pages/contact`; configured social hooks remain available without inventing empty icons or claims.
...
The user selected research/design/previews/diptyque-direction-1.png and later clarified that the full Diptyque homepage rhythm is required. The homepage is expanded through layering, Dukhoon, gifting, wardrobe and service sections; do not collapse it back to only the opening sections. Main was connected to the verified test storefront https://dk5qhx-ax.myshopify.com/, but its trial ended by 2026-09-16. The repository now also has a Vercel-only static visual build (`npm run preview:build`); the current catalog-free public deployment is https://alma-fragrances-preview-9ilk2929v-omarashraafs-projects.vercel.app. Vercel Authentication and password protection are disabled, this surface always renders with an empty catalog, and it is not a commerce emulator. The test store contained 23 products/189 variants and six category collections imported from the public source snapshot; read the currency and inventory limitations above before production work.
