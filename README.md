...
- Website identity: the homepage opening header uses the newly supplied transparent `assets/alma-logo-wordmark.png` above the navigation; after scrolling, and throughout mobile, it uses the previously selected icon from `research/brand/logo-icon.png`. Both source artworks remain exact and product packaging is unchanged.
...
- `snippets/alma-logo.liquid`, `assets/alma-logo-icon.png` and `assets/alma-logo-wordmark.png`: exact supplied transparent artworks at their native dimensions. On the desktop homepage, the 132.5 px opening header centers the wordmark above navigation while keeping utilities at right; after scrolling it becomes a 72 px row with the icon left and navigation centered. The wordmark's transparent source canvas is cropped only by its CSS viewport, not edited. Mobile retains the enlarged icon left-aligned and never shows the opening wordmark. While its menu is open below 990 px, the header is fixed to the current viewport so Dawn's drawer cannot fall back to the header's original document position. `header.liquid` retains Dawn's utility controls, menu drawer, app blocks, account and cart.
