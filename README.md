...
- `sections/footer.liquid` and `assets/alma-footer.js`: preserve Dawn's customer newsletter form, localization, payment and social hooks while adding ALMA Shop, Explore, Help and Legal navigation. The four columns remain expanded on desktop and become accessible native `details` accordions below 750 px. Contact links normalize Shopify's internal page URI to `/pages/contact`; the dark closing bar states the UAE/English storefront context and retains the live shop copyright.
