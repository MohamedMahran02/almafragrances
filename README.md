...
Latest change: homepage vertical spacing remains tighter, while Dukhoon keeps its original full-width desktop photo panel followed by centered story. A generated 16:9 environment fills the wide panel behind the unchanged Shopify portrait product image, reducing the apparent zoom; mobile retains the existing cover crop and title overlay. The generated Dukhoon background, CSS and Liquid were pulled back from the connected Shopify theme and match the repository; visually inspect the wide desktop composite and unchanged mobile treatment. Next requested work: perform final owner review of AED price presentation, the simplified header and repaired storefront destinations on the connected Shopify theme. Typography option 2 (Cormorant Garamond + Jost) remains the active temporary comparison candidate until the user selects it or asks for option 3 (Instrument Serif + Inter).
```
